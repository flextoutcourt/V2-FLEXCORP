...
export default function Flex3d(){
  const ref = useRef()
    return (
        <Canvas shadows dpr={[1, 2]} camera={{ fov: 45, zoom: 3 }} style={{height: "350px"}}>
        <Suspense fallback={null}>
            <Stage controls={ref} preset="rembrandt" intensity={1} environment="city">
                <Model />
            </Stage>
        </Suspense>
        <OrbitControls ref={ref} minPolarAngle={(Math.PI)/2 - 0.3} maxPolarAngle={(Math.PI) / 2 + 0.3} minAzimuthAngle={-Math.PI/2} maxAzimuthAngle={Math.PI/2} />
        </Canvas>
      )
}